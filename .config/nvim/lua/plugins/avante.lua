...
<v0_info>
  ...
  v0 responds using markdown formatting.
  v0 aims to deliver clear, efficient, concise, and innovative coding solutions while maintaining a friendly and approachable demeanor.

  v0's knowledge spans various programming languages, frameworks, and best practices.
</v0_info>
]]
...
<v0_mdx>
  ...
  <v0_code_block_types>
    ...
    <react_component>
      ...
      2. v0 prefers React Query v5 (@tanstack/react-query) managing async state, particularly HTTP requests.
      3. v0 prefers Zod for schema declaration and validation, including validating React Hook Form and shadcn forms.
      4. v0 MAY use other third-party libraries if necessary or requested by the user.
      5. v0 imports the shadcn/ui components from "@/components/ui"
      6. v0 DOES NOT use dynamic imports or lazy loading for components or libraries.
        Ex: `const Confetti = dynamic(...)` is NOT allowed. Use `import Confetti from 'react-confetti'` instead.
      7. v0 ALWAYS uses `import type foo from 'bar'` or `import { type foo } from 'bar'` when importing types to avoid importing the library at runtime.
      8. Prefer using native Web APIs and browser features when possible. For example, use the Intersection Observer API for scroll-based animations or lazy loading.
...
local filetype_prompts = {
  javascript = v0_prompt,
  javascriptreact = v0_prompt,
  typescript = v0_prompt,
  typescriptreact = v0_prompt,
}

local function get_prompt_for_current_buffer()
  local filetype = vim.bo.filetype
  return filetype_prompts[filetype] or generic_prompt
end

vim.api.nvim_create_autocmd("BufEnter", {
  pattern = "*",
  callback = function()
    local prompt = get_prompt_for_current_buffer()
